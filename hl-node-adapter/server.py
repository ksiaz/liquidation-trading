...
from readers import PriceReader, LiquidationReader
...
class AdapterServer:
    ...
    def run(self):
        ...
        try:
            while self._running:
                ...
                metrics = self._grpc_server.servicer.get_metrics()
                if metrics['price_subscribers'] > 0 or metrics['liq_subscribers'] > 0:
                    print(f"[SERVER] Subscribers: prices={metrics['price_subscribers']}, "
                          ...
                          f"liqs={metrics['liquidations_broadcast']}", file=sys.stderr)
    ...
    def stop(self):
        ...
        print(f"[SERVER] Stopped. Emitted {self._prices_emitted} prices, "
              f"{self._liquidations_emitted} liquidations.", file=sys.stderr)
