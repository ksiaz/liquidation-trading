"""
Test LiquidityDrainDetector across multiple symbols (BTC, ETH, SOL).
Validates that detector works on different assets, not just overfitted to SOL.
"""
...
def test_symbol(symbol, profile='RELAXED'):
    """Test detector on a specific symbol."""
    ...
    print(f"\n{'='*80}")
    print(f"Testing {symbol} - {len(rows)} snapshots")
    print(f"{'='*80}")
...
def test_all_symbols():
    """Test detector on BTC, ETH, and SOL."""
    
    symbols = ['BTCUSDT', 'ETHUSDT', 'SOLUSDT']
    results = {}
    
    print("="*80)
    print("MULTI-SYMBOL DETECTOR VALIDATION")
    print("Profile: RELAXED")
    print("="*80)
    
    for symbol in symbols:
        result = test_symbol(symbol, profile='RELAXED')
        if result:
            results[symbol] = result
        else:
            print(f"\n⚠️ No data available for {symbol}")
    
    # Summary table
    print("\n" + "="*80)
    print("CROSS-SYMBOL PERFORMANCE")
    print("="*80)
    print(f"{'Symbol':<12} {'Signals':<10} {'Sig/Hr':<10} {'WR%':<10} {'Net PnL':<12} {'Avg Trade':<12}")
    print("-"*80)
    
    for symbol in symbols:
        if symbol in results:
            r = results[symbol]
            print(f"{symbol:<12} {r['signals']:<10} {r['signals_per_hour']:<10.1f} "
    ...
    if results:
        ...
        avg_pnl = sum([r['net_pnl'] for r in results.values()]) / len(results)
        total_signals = sum([r['signals'] for r in results.values()])
        total_trades = sum([r['trades'] for r in results.values()])
        
        print(f"\n📊 AGGREGATE STATISTICS:")
        print(f"  Average Win Rate: {avg_wr:.1f}%")
        print(f"  Average Net PnL: {avg_pnl:+.2f}%")
        print(f"  Total Signals: {total_signals}")
        print(f"  Total Trades: {total_trades}")
        
        # Consistency check
        profitable_symbols = [s for s, r in results.items() if r['net_pnl'] > 0]
        print(f"\n✅ Profitable on {len(profitable_symbols)}/{len(results)} symbols: {', '.join(profitable_symbols)}")
        
        if avg_wr > 55 and avg_pnl > 0:
            print(f"\n🎉 VALIDATION PASSED - Ready for production!")
        else:
            print(f"\n⚠️ Performance below target - needs tuning")
    
    return results

if __name__ == "__main__":
    results = test_all_symbols()
