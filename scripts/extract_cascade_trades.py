...
- Liquidation burst: sum liq_value in [t-10s, t] >= $50,000
...
class CascadeExtractor:
    ...
    def extract_from_database(self) -> List[CascadeEvent]:
        """Extract cascade events from PostgreSQL database.
        ...
        try:
            ...
            cursor.execute("""
                ...
                WHERE schemaname = 'public'
                  AND tablename IN ('hl_cascade_events', 'mark_prices', 'orderbook_depth')
            """)
            ...
            print(f"  Extracted {len(events)} events from database")
            return events
        finally:
            put_conn(conn)
